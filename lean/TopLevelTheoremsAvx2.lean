...
| prefix | world | status |
| ------ | ----- | ------ |
| `RustKopisAvx2.…` | the **extracted Rust** (`ExtractedRustAvx2.lean`, from `../src/*.rs`) | trusted |
...
`RustKopisAvx2.…` is trusted because the charon/aeneas extraction is; `Spec.…` because you have
...
So `RustKopisAvx2.kem.KemSecretKey.expand_from_seed` is Rust, `Spec.Kopis.ExpandSecretKey` is the
...
`RustKopisAvx2.…` name appears to the left of its `⦃ … ⦄`.

The `RustKopisAvx2` name is not what the Rust crate is called — the crate is `kopis`. It is
...
One more decoding note. Names like `RustKopisAvx2.impls.kopis512.KemSecretKey2` are
...
generic type — `RustKopisAvx2.kem.KemSecretKey 2#usize` — and only the path is
specialized.
...
vectors), and the charon/aeneas extraction really did produce `ExtractedRustAvx2.lean`
from `../src/*.rs`.
-/
...
theorem arrayToBytes_is_identity {n : Usize} (a : Array U8 n) :
  ...
  Kopis.Avx2.Properties.arrayToBytes_toList a
...
theorem pk_serialize_matches_translation {L : Usize} (self : RustKopisAvx2.pke.PkePublicKey L)
    (out_buf : Slice U8) (hlen : out_buf.val.length = L.val * 320 + 32)
    (hfit : L.val * 10 * 256 ≤ Usize.max) :
    RustKopisAvx2.pke.PkePublicKey.serialize self out_buf
      ⦃ (r : Slice U8) => r.length = L.val * (32 * 10) + 32 ∧
          r.val.map (·.bv)
            = (Properties.vecBytesFlat self).toList
            ++ (Properties.arrayToBytes self.matrix_seed).toList ⦄ :=
  Kopis.Avx2.Properties.pke_serialize_spec self out_buf hlen hfit
...
theorem kopis512_keygen (seed : Array U8 32#usize) :
    RustKopisAvx2.kem.KemSecretKey.expand_from_seed 2#usize 10#usize seed
      ⦃ (ksk : RustKopisAvx2.kem.KemSecretKey 2#usize) =>
  ...
  apply Aeneas.Std.WP.spec_mono (Kopis.Avx2.Properties.expand_from_seed_spec 2#usize 10#usize seed
...
theorem kopis768_keygen (seed : Array U8 32#usize) :
    RustKopisAvx2.kem.KemSecretKey.expand_from_seed 3#usize 8#usize seed
      ⦃ (ksk : RustKopisAvx2.kem.KemSecretKey 3#usize) =>
          -- the spec's key-expansion output, with its observable components named
          ...
          Properties.arrayToBytes ksk.kem_pk.hash_pke_pk = pkHash ⦄ := by
  apply Aeneas.Std.WP.spec_mono (Kopis.Avx2.Properties.expand_from_seed_spec 3#usize 8#usize seed
...
theorem kopis1024_keygen (seed : Array U8 32#usize) :
    RustKopisAvx2.kem.KemSecretKey.expand_from_seed 4#usize 6#usize seed
      ⦃ (ksk : RustKopisAvx2.kem.KemSecretKey 4#usize) =>
          -- the spec's key-expansion output, with its observable components named
          ...
          Properties.arrayToBytes ksk.kem_pk.hash_pke_pk = pkHash ⦄ := by
  apply Aeneas.Std.WP.spec_mono (Kopis.Avx2.Properties.expand_from_seed_spec 4#usize 6#usize seed
...
theorem kopis512_keygen_then_encapsulate (seed randomness : Array U8 32#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 2#usize 10#usize seed
        let kpk ← RustKopisAvx2.impls.kopis512.KemSecretKey2.public_key ksk
        RustKopisAvx2.impls.kopis512.KemPublicKey2.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 736#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
  ...
  Kopis.Avx2.Properties.kopis512_keygen_encap_spec seed randomness
...
theorem kopis768_keygen_then_encapsulate (seed randomness : Array U8 32#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 3#usize 8#usize seed
        let kpk ← RustKopisAvx2.impls.kopis768.KemSecretKey3.public_key ksk
        RustKopisAvx2.impls.kopis768.KemPublicKey3.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 1088#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
          -- the spec's encapsulation, with its two components named
          ...
          Properties.arrayToBytes ct = specCt ∧
          Properties.arrayToBytes ss = specSs ⦄ :=
  Kopis.Avx2.Properties.kopis768_keygen_encap_spec seed randomness
...
theorem kopis1024_keygen_then_encapsulate (seed randomness : Array U8 32#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 4#usize 6#usize seed
        let kpk ← RustKopisAvx2.impls.kopis1024.KemSecretKey4.public_key ksk
        RustKopisAvx2.impls.kopis1024.KemPublicKey4.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 1472#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
          -- the spec's encapsulation, with its two components named
          ...
          Properties.arrayToBytes ct = specCt ∧
          Properties.arrayToBytes ss = specSs ⦄ :=
  Kopis.Avx2.Properties.kopis1024_keygen_encap_spec seed randomness
...
theorem kopis512_from_bytes_then_encapsulate (pk_bytes : Array U8 672#usize)
    ...
    (do let kpk ← RustKopisAvx2.impls.kopis512.KemPublicKey2.from_bytes pk_bytes
        RustKopisAvx2.impls.kopis512.KemPublicKey2.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 736#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
          -- the spec's encapsulation, with its two components named
          ...
          Properties.arrayToBytes ct = specCt ∧
          Properties.arrayToBytes ss = specSs ⦄ :=
  Kopis.Avx2.Properties.kopis512_from_bytes_encap_spec pk_bytes randomness
...
theorem kopis768_from_bytes_then_encapsulate (pk_bytes : Array U8 992#usize)
    (randomness : Array U8 32#usize) :
    (do let kpk ← RustKopisAvx2.impls.kopis768.KemPublicKey3.from_bytes pk_bytes
        RustKopisAvx2.impls.kopis768.KemPublicKey3.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 1088#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
          -- the spec's encapsulation, with its two components named
          ...
          Properties.arrayToBytes ct = specCt ∧
          Properties.arrayToBytes ss = specSs ⦄ :=
  Kopis.Avx2.Properties.kopis768_from_bytes_encap_spec pk_bytes randomness
...
theorem kopis1024_from_bytes_then_encapsulate (pk_bytes : Array U8 1312#usize)
    (randomness : Array U8 32#usize) :
    (do let kpk ← RustKopisAvx2.impls.kopis1024.KemPublicKey4.from_bytes pk_bytes
        RustKopisAvx2.impls.kopis1024.KemPublicKey4.encapsulate_deterministic kpk randomness)
      ⦃ (ct : Array U8 1472#usize) (ss : RustKopisAvx2.kem.SharedSecret) =>
          -- the spec's encapsulation, with its two components named
          ...
          Properties.arrayToBytes ct = specCt ∧
          Properties.arrayToBytes ss = specSs ⦄ :=
  Kopis.Avx2.Properties.kopis1024_from_bytes_encap_spec pk_bytes randomness
...
theorem kopis512_keygen_then_decapsulate (seed : Array U8 32#usize) (ek : Array U8 736#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 2#usize 10#usize seed
        RustKopisAvx2.impls.kopis512.KemSecretKey2.decapsulate ksk ek)
  ...
  Kopis.Avx2.Properties.kopis512_keygen_decap_spec seed ek
...
theorem kopis768_keygen_then_decapsulate (seed : Array U8 32#usize) (ek : Array U8 1088#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 3#usize 8#usize seed
        RustKopisAvx2.impls.kopis768.KemSecretKey3.decapsulate ksk ek)
      ⦃ (r : RustKopisAvx2.kem.SharedSecret) =>
          Properties.arrayToBytes r
            = Spec.Kopis.KemDecap .Kopis_768 (Properties.arrayToBytes seed)
                (Properties.arrayToBytes ek) ⦄ :=
  Kopis.Avx2.Properties.kopis768_keygen_decap_spec seed ek
...
theorem kopis1024_keygen_then_decapsulate (seed : Array U8 32#usize) (ek : Array U8 1472#usize) :
    (do let ksk ← RustKopisAvx2.kem.KemSecretKey.expand_from_seed 4#usize 6#usize seed
        RustKopisAvx2.impls.kopis1024.KemSecretKey4.decapsulate ksk ek)
      ⦃ (r : RustKopisAvx2.kem.SharedSecret) =>
          Properties.arrayToBytes r
            = Spec.Kopis.KemDecap .Kopis_1024 (Properties.arrayToBytes seed)
                (Properties.arrayToBytes ek) ⦄ :=
  Kopis.Avx2.Properties.kopis1024_keygen_decap_spec seed ek
