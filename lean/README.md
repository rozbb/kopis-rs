...
make prove-kopis-avx2    # the AVX2 extraction + its assumed intrinsic semantics
make test-kopis-spec     # lake exe kopisTests — run the spec against test vectors
```

`TopLevelTheoremsAvx2.lean` is generated from `TopLevelTheoremsSerial.lean` by
swapping the backend names — the top-level statements are the same for every
backend, so an auditor reads one file rather than one per backend. It is checked
in, and `make prove-kopis` regenerates it and fails if that changed anything, so
the committed copy cannot quietly go stale. Do not edit it by hand.
...
| `TopLevelTheoremsAvx2.lean`| —      | **Autogenerated. Do not edit.** The same statements with the backend names swapped; `make prove-kopis` keeps it current. Not yet built — there are no AVX2 proofs to discharge it. |
