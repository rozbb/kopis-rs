/-
  # Kopis/Neon/SerGeneric.lean — the portable unpacker, in the NEON namespace.

  `neon_deserialize_eq` compares two functions that live in the *same* extraction, so it needs
  no theorem about the serial backend — but it does need this one, because
  `RustKopisNeon.ser.deserialize_generic` is a different Lean constant from
  ...
  (width 13) with `RustKopisSerial` replaced by `RustKopisNeon`, and nothing else: they mention
