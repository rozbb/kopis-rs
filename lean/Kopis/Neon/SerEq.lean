/-
  # Kopis/Neon/SerEq.lean — Phase C's target.

  The NEON bit-unpacker and the portable one produce the same array, for every coefficient width
  the crate uses:

      backend::neon::ser::deserialize bytes bits  =  ser::deserialize_generic bytes bits
  ...
  This is what makes the NEON deserializer covered by the portable proofs: anything already
  proved about `ser::deserialize_generic` transfers to `backend::neon::ser::deserialize` by
  rewriting with this.
-/
...
theorem neon_deserialize_eq (bytes : Slice U8) (n : ℕ) (hn1 : 1 ≤ n) (hn13 : n ≤ 13)
    (hlen : bytes.length = 32 * n) (bitsU : Usize) (hbitsU : bitsU.val = n) :
    backend.neon.ser.deserialize bytes bitsU = ser.deserialize_generic 256#usize bytes bitsU := by
