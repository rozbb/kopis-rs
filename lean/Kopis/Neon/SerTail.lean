/-
  # Kopis/Neon/SerTail.lean — where each group's sixteen bytes come from.
  ...
  `32·bits`-byte buffer.  `src/backend/neon/ser.rs` copies that short remainder into a
  zero-padded 32-byte scratch buffer and reads those groups from there.

  Both paths are made to say the same thing:
  ...
  The geometry is identical to AVX2's — the buffer, the group size and the 16-byte load are the
  same, because a NEON register is exactly the 128-bit half an AVX2 group was broadcast from.
  What is *not* here is `broadcast_bytes`: there is no broadcast, so the loaded register's bytes
  are the group's bytes outright rather than its bytes modulo 16.
-/
