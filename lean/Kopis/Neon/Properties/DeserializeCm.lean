-- AUTOGENERATED from Kopis/Properties/DeserializeCm.lean by `make generated`.  Do not edit.
...
theorem ringElem_deserialize_gen_spec (bytes : Slice U8) (n : ℕ) (hn : 1 ≤ n ∧ n ≤ 12)
  ...
  obtain ⟨hn1, hn2⟩ := hn
  have hnv : (n#usize).val = n := by simp
  -- The dispatch is proved once in `Kopis/Neon/SerDispatch.lean`; here it is instantiated at
  -- the generic widths, with the portable branch supplied by the twin's own generic decoder.
  have hstream := Kopis.Neon.ringElem_deserialize_gen_streamNat bytes n hn1 hn2 hne10 hlen
    (Kopis.Neon.Generic.generic_streamNat bytes n hn1 (by omega) hlen)
