-- AUTOGENERATED from Kopis/Properties/Ntt.lean by `make generated`.  Do not edit.
...
private theorem ntt_from_bytes_raw (bytes : Slice U8) (hlen : bytes.length = 32 * 13) :
  ...
  refine WP.spec_mono (Kopis.Neon.ringElem_deserialize_13_streamNat bytes hlen ?_) ?_
  · intro arr harr
...
theorem gen_matrix_uniformBounded {L : Usize} (seed : Array U8 32#usize)
  ...
  obtain ⟨bU, hbU⟩ : ∃ b, backend.neon.cpu.available = ok b := ⟨true, rfl⟩
  rw [hbU, bind_tc_ok]
  have havx : backend.neon.sample.gen_matrix_from_seed L seed
      ⦃ (r : arithmetic.matrix_arith.Matrix L L) => UniformBounded r ⦄ := by
    apply WP.spec_mono
      (Kopis.Neon.Properties.neon_gen_matrix_from_seed_bd L seed (by omega))
...
private theorem ntt_ringElem_deser10_raw (bytes : Slice U8) (hlen : bytes.length = 32 * 10) :
  ...
  refine WP.spec_mono (Kopis.Neon.ringElem_deserialize_10_streamNat bytes hlen ?_) ?_
  · intro arr harr
    -- SerDispatch states the window spec with `[j]!`; this file's spec uses `[j]`.
