-- AUTOGENERATED from Kopis/Properties/GenSecretTop.lean by `make generated`.  Do not edit.
...
import Kopis.Neon.SecretBridge
  ...
  have havx : backend.neon.sample.gen_secret_from_seed L MU seed
      ⦃ (r : arithmetic.matrix_arith.Matrix L 1#usize) =>
          ∀ a (_ha : a < L.val) c (_hc : c < 256),
            smallSignedU16 (((r.val[a]!).val[0]!).val[c]!) (MU.val / 2) ⦄ := by
    apply WP.spec_mono
      (Kopis.Neon.Properties.neon_gen_secret_from_seed_bd L MU seed hMU (by scalar_tac))
    intro r hr
    exact hr
  cases bNeon
  case true => simpa only [reduceIte] using havx
