/-
  # Kopis/Avx2/SerPlan.lean — the constant tables the AVX2 deserializer indexes.

  `src/backend/avx2/ser.rs` builds, at compile time, one `Plan` per coefficient width: a
  `vpshufb` control naming, for 32-bit lane `k`, the four bytes starting at that coefficient's
  first byte, and the matching `vpsrlvd` count.  `PLANS` is that table for widths `0..=13`.

  Nothing here is deep — it is `⌊k·w/8⌋` and `(k·w) % 8` written into arrays by two nested
  loops — but the deserializer's correctness is stated in terms of those two expressions, so
  the table has to be pinned down before anything else can be said.  Three loop specs, in the
  order the code nests them:

  * `plan_inner_spec`   — the four bytes of one lane's shuffle control;
  * `plan_outer_spec`   — all eight lanes, shuffle and shift together;
...
instance : Inhabited backend.avx2.ser.Plan :=
...
theorem plan_outer_spec (bits : Usize) (hbits : bits.val ≤ 13)
    ...
    backend.avx2.ser.plan_loop0 bits shuffle shift k
  ...
  unfold backend.avx2.ser.plan_loop0
  ...
  · rw [if_pos hlt]
    ...
    · rw [hr2 m hm, ha, Array.set_val_eq,
        getElem!_list_set shift.val k.val i2 m (by simpa using hk)]
      by_cases hmk : k1.val ≤ m
      · rw [if_pos hmk, if_pos (by scalar_tac)]
      · rw [if_neg hmk]
        by_cases hmeq : m = k.val
        · rw [if_pos hmeq, if_pos (by omega), hi2, hi1v, hmeq]
...
theorem plans_loop_spec (plans : Array backend.avx2.ser.Plan 14#usize) (b : Usize)
    (hb : 1 ≤ b.val) :
    backend.avx2.ser.plans_loop plans b
      ⦃ (r : Array backend.avx2.ser.Plan 14#usize) =>
  ...
  unfold backend.avx2.ser.plans_loop
  by_cases hle : b ≤ backend.avx2.ser.MAX_BITS
  · rw [if_pos hle]
    have hb13 : b.val ≤ 13 := by
      simp only [backend.avx2.ser.MAX_BITS] at hle; scalar_tac
    ...
    · rcases Nat.lt_or_ge w b1.val with h | h
      ...
      · exact hprop w h hhi
  · rw [if_neg hle]
    have : 13 < b.val := by simp only [backend.avx2.ser.MAX_BITS] at hle; scalar_tac
    exact ⟨fun w _ => rfl, fun w hlo hhi => by omega⟩
termination_by 14 - b.val
decreasing_by
  simp only [backend.avx2.ser.MAX_BITS] at hle
  scalar_decr_tac

/-- **The table.**  For every width the crate uses, `PLANS[w]` holds the shuffle control and
shift the deserializer's correctness is stated through. -/
theorem PLANS_spec :
    backend.avx2.ser.PLANS
      ⦃ (r : Array backend.avx2.ser.Plan 14#usize) => ∀ w, 1 ≤ w → w ≤ 13 →
          (∀ j < 32, ((r.val[w]!).shuffle.val[j]!).val = planShuffle w (j / 4) (j % 4)) ∧
          (∀ m < 8, ((r.val[w]!).shift.val[m]!).val = (planShift w m : ℤ)) ⦄ := by
  simp only [backend.avx2.ser.PLANS]
  unfold backend.avx2.ser.plans
