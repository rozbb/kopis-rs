/-
  # Kopis/Avx2/SampleBridge.lean — the AVX2 sampling loops against the spec.

  `Keccak/Conform.lean` proves `keccak::xof4` is TurboSHAKE.  This file is what sits on top: the
  AVX2 `sample` module hashes four matrix entries at a time through `xof4`, and the twins in
  `Kopis/Avx2/Properties/` need to know that produces the same matrix the portable path does.

  The batching is where the two paths differ.  The portable path hashes one entry per call; the
  AVX2 path fills four lanes and clamps the trailing ones of a short batch to the last entry, so
...
`.val` level is what lets the AVX2 sampler be proved correct for *every* `L` — including
...
/-- The entry a given lane of a four-wide batch samples: the batch's `first + lane`, clamped to
...
theorem gen_matrix_from_seed_loop0_loop0_spec (L : Std.Usize)
    ...
    (indices : Std.Array (Std.Array Std.U8 2#usize) 4#usize)
    (hL : 0 < L.val) (hmaxe : entries.val + 4 ≤ Std.Usize.max) (hend : iter.«end».val = 4)
    (hstart : iter.start.val ≤ 4) (hent : 0 < entries.val) (hentL : entries.val = L.val * L.val)
    ...
    (hpre : ∀ lane < iter.start.val,
      ...
      ((indices.val[lane]!).val[1]!).bv
        = ((batchEntry entries.val first.val lane % L.val : ℕ) : Byte)) :
    backend.avx2.sample.gen_matrix_from_seed_loop0_loop0 L iter entries first indices
      ⦃ (r : Std.Array (Std.Array Std.U8 2#usize) 4#usize) => ∀ lane < 4,
          ((r.val[lane]!).val[0]!).bv
            = ((batchEntry entries.val first.val lane / L.val : ℕ) : Byte) ∧
          ((r.val[lane]!).val[1]!).bv
            = ((batchEntry entries.val first.val lane % L.val : ℕ) : Byte) ⦄ := by
  unfold backend.avx2.sample.gen_matrix_from_seed_loop0_loop0
  ...
  · let* ⟨ o, iter1, ho, hstart', hend' ⟩ ← core.iter.range.IteratorRange.next_Usize_some_spec
    ...
    have hilen : (indices.val : List (Std.Array Std.U8 2#usize)).length = 4 := indices.property
  ...
  · let* ⟨ o, iter1, hnone, _ ⟩ ← core.iter.range.IteratorRange.next_Usize_none_spec
    ...
    intro lane hlane
    have hge : 4 ≤ iter.start.val := by omega
    exact hpre lane (by omega)
...
`xof4` buffer into its matrix slot; lanes past the end of the matrix are skipped, and every other
slot is left alone. -/
theorem gen_matrix_from_seed_loop0_loop1_spec {L : Std.Usize}
    ...
    (bufs : Std.Array (Std.Array Std.U8 416#usize) 4#usize)
    (hL : 0 < L.val) (hend : iter.«end».val = 4) (hstart : iter.start.val ≤ 4)
    (hentL : entries.val = L.val * L.val) (hfirst : first.val < entries.val)
    (hmax : first.val + 4 ≤ Std.Usize.max) :
    backend.avx2.sample.gen_matrix_from_seed_loop0_loop1 iter mat entries first bufs
      ⦃ (r : arithmetic.matrix_arith.Matrix L L) => ∀ a < L.val, ∀ b < L.val, ∀ j < 256,
          (((r.val[a]!).val[b]!).val[j]!).val =
            if first.val + iter.start.val ≤ a * L.val + b ∧ a * L.val + b < first.val + 4
  ...
  have hmeb : backend.avx2.sample.MATRIX_ELEM_BYTES ⦃ (r : Std.Usize) => r.val = 416 ⦄ := by
    ...
    omega
  unfold backend.avx2.sample.gen_matrix_from_seed_loop0_loop1
  by_cases hlt : iter.start.val < iter.«end».val
  · let* ⟨ o, iter1, ho, hstart', hend' ⟩ ← core.iter.range.IteratorRange.next_Usize_some_spec
    rw [ho]; simp only
    have hl4 : iter.start.val < 4 := by scalar_tac
    ...
    · rw [if_pos hin]
      ...
      have hslice : core.slice.index.SliceIndexRangeToUsizeSlice.index
          ({ «end» := meb } : core.ops.range.RangeTo Std.Usize) (Std.Array.to_slice buf)
        ...
        rw [← getElem!_pos _ k hk1, ← getElem!_pos _ k hk2,
          List.getElem!_slice 0 meb.val k buf.val (by rw [hmebv]; simp at hk2 ⊢; omega)]
        simp
      let* ⟨ sl, hsl ⟩ ← hslice
      ...
      by_cases h1 : first.val + (iter.start.val + 1) ≤ a * L.val + b
          ∧ a * L.val + b < first.val + 4 ∧ a * L.val + b < entries.val
      ...
      · rw [if_neg h1, hread a ha b hb]
        ...
        · have habe : a * L.val + b = entry.val := by rw [h2.1, h2.2]; exact hqm
          ...
          exact Subtype.ext hsl
        · have hne : ¬(first.val + iter.start.val ≤ a * L.val + b
              ∧ a * L.val + b < first.val + 4 ∧ a * L.val + b < entries.val) := by
            rintro ⟨hc1, hc2, hc3⟩
...
/-- The message `xof4` hashes for entry `(a, b)` is the spec's `seed ‖ a ‖ b`. -/
...
/-- **A lane's `xof4` buffer is the spec's hash for that entry.**  Reading the buffer as a spec
...
/-- **The batching loop.**  Four entries per `xof4` call; every entry from `first` on ends up
...
theorem gen_matrix_from_seed_loop0_spec {L : Std.Usize} (seed : Std.Array Std.U8 32#usize)
    (mat : arithmetic.matrix_arith.Matrix L L) (entries first : Std.Usize)
    (hentL : entries.val = L.val * L.val) (hmax : entries.val + 4 ≤ Std.Usize.max) :
    backend.avx2.sample.gen_matrix_from_seed_loop0 seed mat entries first
  ...
  unfold backend.avx2.sample.gen_matrix_from_seed_loop0
  ...
  · rw [if_pos hlt]
    ...
    let* ⟨ indices1, hidx ⟩ ← gen_matrix_from_seed_loop0_loop0_spec L
      ...
      entries first (Std.Array.repeat 4#usize (Std.Array.repeat 2#usize 0#u8))
      hL hmax rfl (by simp) (by omega) hentL hfv (by intro lane hlane; simp at hlane)
    obtain ⟨bufs1, hb1, hp0⟩ := WP.spec_imp_exists
      (xof4_turboSHAKE 168#usize 2#u8 seed indices1
        (Std.Array.repeat 4#usize (Std.Array.repeat 416#usize 0#u8)) 0 (by omega)
        (Or.inl rfl) (by decide) (by decide) (by decide) (by scalar_tac))
    rw [hb1, bind_tc_ok]
    have hlane : ∀ l, l < 4 → ∀ j < 416,
        ((bufs1.val[l]!).val[j]!).bv
          = (turboSHAKE128 (msgOf seed (indices1.val[l]!)) (2#u8).bv 416)[j]! := by
      intro l hl
      have h := xof4_turboSHAKE 168#usize 2#u8 seed indices1
        (Std.Array.repeat 4#usize (Std.Array.repeat 416#usize 0#u8)) l hl
        (Or.inl rfl) (by decide) (by decide) (by decide) (by scalar_tac)
      rw [hb1] at h
    ...
    let* ⟨ mat1, hm1 ⟩ ← gen_matrix_from_seed_loop0_loop1_spec
      ...
      hL rfl (by simp) hentL hfv (by omega)
    let* ⟨ first1, hf1 ⟩ ← Std.Usize.add_spec (x := first) (y := 4#usize) (by scalar_tac)
    apply WP.spec_mono
      (gen_matrix_from_seed_loop0_spec seed mat1 entries first1 hentL hmax)
    ...
    rw [hr a ha b hb, hf1]
    by_cases h4 : first.val + 4 ≤ a * L.val + b
    ...
    · rw [if_neg h4]
      ...
      · rw [if_pos h0]
        ...
        have hamod : (a * L.val + b) % L.val = b := by
          rw [Nat.mul_comm, Nat.add_comm, Nat.add_mul_mod_self_left, Nat.mod_eq_of_lt hb]
        have hlane4 : a * L.val + b - first.val < 4 := by omega
...
/-- **`backend::avx2::sample::gen_matrix_from_seed` matches the spec.**  Every entry is the
spec's hash-derived polynomial for its coordinates. -/
theorem avx2_gen_matrix_from_seed_spec (L : Std.Usize) (seed : Std.Array Std.U8 32#usize)
    (hmax : L.val * L.val + 4 ≤ Std.Usize.max) :
    backend.avx2.sample.gen_matrix_from_seed L seed
      ⦃ (r : arithmetic.matrix_arith.Matrix L L) => ∀ a < L.val, ∀ b < L.val,
          toRingElem13 ((r.val[a]!).val[b]!) = entryOf seed a b ⦄ := by
  unfold backend.avx2.sample.gen_matrix_from_seed
  ...
  apply WP.spec_mono (gen_matrix_from_seed_loop0_spec seed _ entries 0#usize
...
theorem gen_matrix_from_seed_loop0_bd {L : Std.Usize} (seed : Std.Array Std.U8 32#usize)
  ...
  · rw [if_pos hlt]
    ...
    let* ⟨ indices1, hidx ⟩ ← gen_matrix_from_seed_loop0_loop0_spec L
      ...
      hL hmax rfl (by simp) (by omega) hentL hfv (by intro lane hlane; simp at hlane)
    obtain ⟨bufs1, hb1, hp0⟩ := WP.spec_imp_exists
      (Kopis.Avx2.Keccak.xof4_turboSHAKE 168#usize 2#u8 seed indices1
        (Std.Array.repeat 4#usize (Std.Array.repeat 416#usize 0#u8)) 0 (by omega)
        (Or.inl rfl) (by decide) (by decide) (by decide) (by scalar_tac))
    rw [hb1, bind_tc_ok]
    let* ⟨ mat1, hm1 ⟩ ← gen_matrix_from_seed_loop0_loop1_spec
    ...
    let* ⟨ first1, hf1 ⟩ ← Std.Usize.add_spec (x := first) (y := 4#usize) (by scalar_tac)
    apply WP.spec_mono
      (gen_matrix_from_seed_loop0_bd seed mat1 entries first1 hentL hmax ?_)
    · intro r hr
      exact hr
    · intro a b c ha hb hc hlt2
      ...
      by_cases hw : first.val + 0 ≤ a * L.val + b ∧ a * L.val + b < first.val + 4
          ∧ a * L.val + b < entries.val
      · rw [if_pos hw]
...
/-- **`backend::avx2::sample::gen_matrix_from_seed`'s coefficient bound.** -/
theorem avx2_gen_matrix_from_seed_bd (L : Std.Usize) (seed : Std.Array Std.U8 32#usize)
    (hmax : L.val * L.val + 4 ≤ Std.Usize.max) :
    backend.avx2.sample.gen_matrix_from_seed L seed
      ⦃ (r : arithmetic.matrix_arith.Matrix L L) => ∀ a b c, a < L.val → b < L.val → c < 256 →
          (((r.val[a]!).val[b]!).val[c]!).val < 2 ^ 13 ⦄ := by
  ...
  let* ⟨ entries, hent ⟩ ← Std.Usize.mul_spec (x := L) (y := L) (by scalar_tac)
  apply WP.spec_mono
    (gen_matrix_from_seed_loop0_bd seed _ entries 0#usize (by rw [hent]) (by rw [hent]; omega)
