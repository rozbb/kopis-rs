/-
  # Kopis/Avx2/CbdEq.lean — Phase D's target.

  The AVX2 centred-binomial sampler and the portable one produce the same ring element:
  ...
  gives the AVX2 coefficient as a *raw* `u16` (`cbdU16`), while the portable chain gives its
...
crate instantiates. -/
theorem avx2_cbd_eq (buf : Slice U8) (MU : Usize) (out : RingElem)
    (hMU : MU.val = 6 ∨ MU.val = 8 ∨ MU.val = 10)
    (hlen : buf.val.length = 32 * MU.val) :
    backend.avx2.sample.cbd MU buf = sample.cbd MU buf out := by
