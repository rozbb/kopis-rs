/-
  # Kopis/Avx2/Keccak/SpecState.lean — the register state and the sponge state are the same state.
  ...
  * §3.1.2 (`stringToState`) puts bit `z` of lane `(x, y)` at position `64·(5y + x) + z`.
  ...
  lane at.  There is no byte reversal at any step, and `Absorb.lean`'s `leWord64` is the same
