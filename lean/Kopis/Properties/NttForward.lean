...
/-! ## The closing Barrett pass

`ntt_loop1` maps `barrett_reduce` over the coefficient slice, re-centring every coefficient to
`(-p, p)`.  Unlike the butterfly loops this is an `IterMut` loop: it threads a continuation
`back` that performs the writes.  The invariant is stated in the same style as
`MatrixArith.shift_right_loop0_loop0_spec`, which is the model for this proof. -/

/-- The reduced-coefficient predicate the Barrett pass establishes. -/
def BarrettOK (x orig : ℤ) : Prop := x ≡ orig [ZMOD pNtt] ∧ -pNtt < x ∧ x < pNtt

theorem ntt_barrett_loop_spec
    ...
    arithmetic.ntt.ntt_loop1 iter back
  ...
  unfold arithmetic.ntt.ntt_loop1
  ...
  · let* ⟨ o, iter1, next_back, h_all ⟩ ← iter_mut_next_spec
    ...
    apply WP.spec_mono
      (ntt_barrett_loop_spec iter1 (fun im => back (next_back im (some coeff1))) orig B
