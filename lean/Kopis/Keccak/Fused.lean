/-
  # Kopis/Keccak/Fused.lean — the fused ρ-π-χ, and the tables `keccak.rs` hard-codes.

  `Round.lean` shows the FIPS round is `RndW = ιW ∘ χW ∘ πW ∘ ρW ∘ θW` on 64-bit words.
  `each backend's `keccak.rs`` does not evaluate those five in sequence.  It computes θ's five
